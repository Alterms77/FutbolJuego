...
namespace FutbolJuego.Utils
{
    ...
    /// Resources/Localisation/{languageCode}.json at runtime.
    /// </summary>
    public class LocalizationManager
    {
        ...
        public static string Get(string key)
        {
            ...
            Debug.LogWarning($"[Localisation] Missing key '{key}' for language '{CurrentLanguage}'.");
            return key;
        }
        ...
        public static void SetLanguage(string languageCode)
        {
            ...
            // Try to load from Resources/Localisation/{code}.json
            var asset = Resources.Load<TextAsset>($"Localisation/{languageCode}");
            if (asset == null)
            {
                Debug.LogWarning($"[Localisation] No file found for language '{languageCode}'. Using fallback.");
                LoadFallback();
                return;
            }
            ...
            try
            {
                var wrapper = JsonUtility.FromJson<LocalisationWrapper>(asset.text);
                currentStrings = new Dictionary<string, string>();
                if (wrapper?.entries != null)
                    foreach (var entry in wrapper.entries)
                        currentStrings[entry.key] = entry.value;

                Debug.Log($"[Localisation] Loaded {currentStrings.Count} strings for '{languageCode}'.");
            }
            catch
            {
                Debug.LogError($"[Localisation] Failed to parse localisation file for '{languageCode}'.");
                LoadFallback();
            }
        }
