...
namespace FutbolJuego.Systems
{
    ...
    public class EconomySystem
    {
        ...
        public bool CheckBankruptcy(TeamData team)
        {
            ...
            {
                GD.PushWarning($"[EconomySystem] {team.name} is BANKRUPT! Balance: {team.finances.balance}");
                return true;
            }
            return false;
        }
        ...
        public bool UpgradeInfrastructure(TeamData team, InfrastructureType infrastructureType)
        {
            ...
            if (currentLevel >= 5)
            {
                GD.Print($"[EconomySystem] {name} is already at maximum level.");
                return false;
            }

            if (!team.finances.CanAfford(cost))
            {
                GD.Print($"[EconomySystem] Cannot afford {name} upgrade. Cost: {cost}, Balance: {team.finances.balance}");
                return false;
            }
            ...
            GD.Print($"[EconomySystem] {team.name} upgraded {name} to level {currentLevel + 1}.");
            return true;
        }
    }
}
